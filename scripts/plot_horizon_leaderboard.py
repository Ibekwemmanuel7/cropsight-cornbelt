...
def main() -> int:
    ...
    ax = axes[0, 0]
    ax.plot(
        val["week_k"], val["rmse"], marker="o", linewidth=2.0, label="Val (2022)", color="#1f3864"
    )
    ax.plot(
        test["week_k"],
        test["rmse"],
    ...
    ax = axes[1, 0]
    ax.plot(test["week_k"], test["mean_width"], marker="s", linewidth=2.0, color="#7b1fa2")
    ax.set_xlabel("Forecast week K")
    ...
    ax.bar(counts["week_k"], counts["n_features"], width=2.5, color="#4472c4", alpha=0.8)
    ax.set_xlabel("Forecast week K")
